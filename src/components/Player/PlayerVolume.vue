...
export default {
  ...
  setup(props, { emit }) {
    ...
    const volumeIconPath = computed(() => {
      ...
      } else if (props.volume <= 0.6 && props.volume > 0) {
    ...
    const tooltipText = computed(() => {
      ...
      return `${text.volume}${volumePer(props.volume, props.total)}%`
    })
    ...
    function initVolume() {
      ...
      changeVolume(storedVolume ? Number(storedVolume) : props.volume)
      changeMuted(storedMuted ? Boolean(Number(storedMuted)) : props.muted)
    }

    function volumeUp() {
      changeVolume(props.volume + 0.05)
    }
    ...
    hotkey('ArrowUp', volumeUp)
    hotkey('ArrowDown', volumeDown)
    hotkey('M', mute)
